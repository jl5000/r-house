...
For the purposes of this blog post I'm also going to make use of the `DT` package for interactive tables and the `widgetframe` package to allow me to embed them on this webpage.
...
frameWidget(datatable(max_stats), height = 500, width = '100%')
```
...
frameWidget(datatable(set_bonus_rules))
```
...
rm(mods_json)
frameWidget(datatable(mods_info))
```
...
frameWidget(datatable(toon_stat_priority))
```
...
frameWidget(datatable(mod_list_shape))
```

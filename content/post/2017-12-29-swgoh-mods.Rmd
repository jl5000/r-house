...
```{r message = FALSE}
library(jsonlite)
mods_json <- fromJSON("swgoh-mods-sample.json")
...
The first code chunk in the loop checks whether there are ghost mods AND ghost toons and then goes through each Ghost mod and permutation about to be generated and ensures it is assigned to a ghost toon (so that the problem space is reduced as much as possible). The second code chunk goes on to randomly assign the other mods (I neglected to set any random number seed, although this is easily fixed).

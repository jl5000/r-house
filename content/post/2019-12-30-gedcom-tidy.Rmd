...
I now find myself in a situation where I'm keen to dig out my old GEDCOM file and see what R can do with it! I searched on GitHub for repos that manipulate GEDCOM files in R, and perhaps the most promising was one by Peter Prevos who had [written a short article](https://lucidmanager.org/gedcom-reader/) describing the format of the file and its limitations. I highly recommend you give it a read.
...
```{r echo=FALSE}
example %>% 
  pivot_longer(-ID, names_to = "attribute", values_to = "value") %>% 
  distinct() %>% 
  arrange() %>% 
  mutate(source = c("A", "A", "A", "B", "B")) %>% 
  knitr::kable()
```
...
I have since discovered an open source genealogy project called [Gramps](https://gramps-project.org) which seems to rely on XML data structures. Sounds promising. I intend to try installing this and seeing how it fares with converting my existing GEDCOM files.

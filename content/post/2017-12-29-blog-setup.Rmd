...
Unfortunately, stopping the server crashes RStudio, so I have to run `options(blogdown.generator.server = TRUE)` before running it to stop it crashing when it's stopped. Helpfully, I've found I can create a .Rprofile file in the website root directory containing this command so it loads every automatically every time I load the project.
